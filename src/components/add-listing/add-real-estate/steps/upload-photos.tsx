...
} from "@/redux/api/add-listing/real-estate-api";
...
export default function AddListingPhotos() {
    ...
    const [
        ...
        { isLoading: createRealEstateListingLoading },
    ] = useCreateRealEstateListingMutation();
    const [uploadRealEstateMedia] = useUploadRealEstateMediaMutation();
    ...
    const handleNext = async () => {
        if (newImgArr.length === 0) {
            ...
            try {
                const response = await createRealEstateListing(newObj);
                ...
                mediaFormData.append("realEstateId", response.data.data.id);
                ...
                await uploadRealEstateMedia(mediaFormData).then((response) => {
                    if (response.error) {
                        responseToast(
                            response,
                            navigate,
                            Routes.private.addListing.realEstate,
                            dispatch
                        );
        ...
        <div className="w-full max-w-[548px] xl:max-w-[648px]">
            <form noValidate onSubmit={(e) => e.preventDefault()}>
                ...
                <CreateListingFooter
                    onBack={() => setStep(4)}
                    onNext={handleNext}
                    disabled={createRealEstateListingLoading}
                />
            </form>
        </div>
    );
